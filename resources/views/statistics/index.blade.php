...
<div class="admin-container">
    ...
    <div class="admin-nav-pills admin-mb-4">
        ...
        <a class="admin-nav-pill" href="{{ route('orders.index') }}">
            <i class="fas fa-shopping-cart"></i> Заказы
        </a>
        <a class="admin-nav-pill active" href="{{ route('statistics.index') }}">
            ...
            <div class="admin-card">
                <div class="admin-card-body">
                    ...
                    <div class="admin-d-flex admin-align-items-center admin-mb-2">
                        <h3 class="admin-mb-0 admin-me-3">{{ $generalStats['conversion_rate'] ?? 0 }}%</h3>
                        <div class="admin-progress admin-flex-1">
                            <div class="admin-progress-bar" style="width: {{ min($generalStats['conversion_rate'] ?? 0, 100) }}%"></div>
                        </div>
                    </div>
                    <div class="admin-text-muted admin-small">
                        Из {{ number_format($generalStats['unique_visitors'] ?? 0) }} посетителей {{ number_format($generalStats['completed_orders'] ?? 0) }} совершили покупку
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="admin-col admin-col-6">
            <div class="admin-card">
                <div class="admin-card-body">
                    ...
                    <h3 class="admin-mb-2">{{ $orderStats['average_processing_time'] ?? 0 }} ч</h3>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadChart(type) {
        ...
        // Загружаем данные
        fetch(`{{ route('statistics.chart-data') }}?bot_id={{ $botId }}&period={{ $period }}&start_date={{ $startDate }}&end_date={{ $endDate }}&chart_type=${type}`)
