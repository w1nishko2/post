...
<div class="admin-container">
    ...
    <div class="admin-card">
        ...
        <div class="admin-card-body">
            <form method="POST" action="{{ route('bot.products.update', [$telegramBot, $product]) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="admin-row">
                    ...
                    <div class="admin-col admin-col-4">
                        ...
                        <div class="admin-form-group">
                            ...
                            <!-- Текущие фотографии -->
                            @if($product->photos_gallery && count($product->photos_gallery) > 0)
                            <div class="current-photos" id="current-photos-container" style="display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 15px;">
                                @foreach($product->photos_gallery as $index => $photoUrl)
                                <div class="photo-card" data-index="{{ $index }}" style="position: relative; width: 120px; height: 120px; border-radius: 8px; overflow: hidden; border: 2px solid {{ $index == $product->main_photo_index ? '#f6ad55' : '#e2e8f0' }}; cursor: pointer; transition: all 0.3s ease;">
                                    <img src="{{ asset('storage/' . ltrim($photoUrl, '/')) }}" style="width: 100%; height: 100%; object-fit: cover;" alt="Фото {{ $index + 1 }}" onerror="this.src='{{ asset('images/no-image.png') }}'; this.onerror=null;">
                                    ...
                                    <div class="photo-overlay" style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.6); display: flex; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.3s ease; z-index: 1;">
                                        @if($index != $product->main_photo_index)
                                        <button type="button" onclick="setMainPhoto({{ $index }})" class="admin-btn admin-btn-sm admin-btn-warning" style="padding: 6px 12px; font-size: 12px; border: none; cursor: pointer;">
                            ...
                            <input type="hidden" name="main_photo_index" id="main_photo_index" value="{{ $product->main_photo_index ?? 0 }}">
                        </div>
                    </div>
                </div>
